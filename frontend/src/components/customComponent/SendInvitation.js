...
class Login extends Component {
  ...
  render() {
    ...
    console.log({ auth: auth.user.agents });
    return (
      <div className="row card card-body mt-4 mb-4">
        <p className="h5 text-center mb-4">Invite Agent</p>
        ...
        <table class="table">
          ...
          <tbody>
            {auth &&
              auth.user &&
              auth.user.agents &&
              auth.user.agents.map((agent, index) => {
                return (
                  <tr>
                    <th scope="row">{index + 1}</th>
                    <td>{agent.email}</td>
                    <td>{agent.verified ? "Verified" : "UnVerified"}</td>
                  </tr>
                );
              })}
          </tbody>
